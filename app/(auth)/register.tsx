...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try {
      const res = await authService.register(fullName.trim(), email.trim(), password, phone.trim() || undefined);
      ...
      <Animated.View style={[s.card, { opacity: cardOpacity, transform: [{ translateY: cardY }] }]}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          >
            ...
            <Field label="Họ và tên" icon="person-outline" value={fullName} onChangeText={setFullName} placeholder="Nguyễn Văn A" />
            <Field label="Email" icon="mail-outline" value={email} onChangeText={setEmail} placeholder="you@example.com" keyboardType="email-address" autoCapitalize="none" />
            <Field label="Số điện thoại (tuỳ chọn)" icon="call-outline" value={phone} onChangeText={setPhone} placeholder="0912 345 678" keyboardType="phone-pad" />
            ...
            <View style={s.field}>
              ...
              <View style={s.inputRow}>
                ...
                <TextInput style={[s.input, { flex: 1 }]} placeholder="Tối thiểu 6 ký tự" placeholderTextColor="#9CA3AF" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} />
            ...
            <View style={s.field}>
              ...
              <View style={s.inputRow}>
                <Ionicons name="shield-checkmark-outline" size={18} color={C.muted} style={s.icon} />
                <TextInput style={[s.input, { flex: 1 }]} placeholder="Nhập lại mật khẩu" placeholderTextColor="#9CA3AF" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry={!showConfirm} />
            ...
            <TouchableOpacity style={[s.btn, loading && s.btnDim]} onPress={handleRegister} disabled={loading} activeOpacity={0.82}>
              {loading
...
function Field({ label, icon, value, onChangeText, placeholder, keyboardType = 'default', autoCapitalize = 'words' }: FieldProps) {
  return (
    <View style={s.field}>
...
const s = StyleSheet.create({
  ...
  blob1: { position: 'absolute', top: -90, right: -90, width: 300, height: 300, borderRadius: 150, backgroundColor: 'rgba(255,255,255,0.07)' },
  blob2: { position: 'absolute', top: 30, right: 60, width: 140, height: 140, borderRadius: 70, backgroundColor: 'rgba(255,255,255,0.05)' },
  blob3: { position: 'absolute', bottom: 10, left: -60, width: 220, height: 220, borderRadius: 110, backgroundColor: 'rgba(255,255,255,0.04)' },
  ...
  appName: {
    ...
    color: C.white, letterSpacing: 1.5, marginBottom: 5,
  },
  ...
  errBox: {
    ...
    backgroundColor: C.errorBg, borderRadius: 10, padding: 12, marginBottom: 14,
  },
